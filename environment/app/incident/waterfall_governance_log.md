...
> **Governance decision (2026-05-12 - #WF-4142)** Lena: step balances, final. Every step in the ledger reports the tranche balance as it stands WHEN THAT STEP RUNS, never the balance at period open. `opening_balance_minor` is the balance the tranche carries as the step begins, with every earlier step of the same period already applied to it, and `closing_balance_minor` is that same figure less whatever principal the step itself pays. An interest step amortises nothing, so its closing balance equals its opening balance -- that holds for a deferred subordinate interest step run after a breached coverage test exactly as it does for one paid in the ordinary course, and the balance it reports is the one the principal steps of that period have already left behind, not the one the period opened on. A principal step reduces the balance by its `paid_minor`, so a turbo redemption diverted after the senior principal steps opens at the balance those steps left and closes below it. A step that names no tranche -- a fee step or the residual step -- carries zero in both fields
